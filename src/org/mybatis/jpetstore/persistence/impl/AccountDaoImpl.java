...
public class AccountDaoImpl implements AccountDao {
    ...
    private static final String QUERY_ACCOUNT_BY_USERNAME =
            "SELECT USERNAME FROM SIGNON WHERE USERNAME=?";

    private static final String QUERY_USERNAME_BY_EMAIL =
            "SELECT USERID FROM ACCOUNT WHERE EMAIL=?";

    private static final String QUERY_AllMyLog_BY_USERNAME =
            "SELECT BEHAVIOR,TIME FROM MYLOG WHERE NAME=?";
    ...
    private static final String INSERT_PROFILE=
            "INSERT INTO PROFILE (LANGPREF, FAVCATEGORY, MYLISTOPT,BANNEROPT,USERID) VALUES (?,?,?,?,?)";

    private static final String INSERT_SIGNON=
            "INSERT INTO SIGNON (PASSWORD,USERNAME) VALUES (?,?)";

    private static final String INSERT_MYLOG=
            "INSERT INTO MYLOG (NAME,BEHAVIOR,TIME) VALUES (?,?,?)";

    private static final String DETELE_ACCOUNT=
            "DELETE account,profile ,signon FROM account,profile ,signon " +
                    "WHERE account.userid = ? AND profile.userid = ? AND signon.username=?";
    ...
    public void insertAccount(Account account) {
        ...
        try {
            ...
            if (result!=0){
                System.out.println("ACCOUNT插入成功");
            }
            DBUtil.closePreparedStatement(preparedStatement);
            DBUtil.closeConnection(connection);
        }catch (Exception e){
            e.printStackTrace();
            System.out.println("ACCOUNT插入失败");
        }
    }
    ...
    public void insertProfile(Account account) {
        ...
        try {
            ...
            result= preparedStatement.executeUpdate();
            if (result!=0){
                System.out.println("PROFILE插入成功");
            }
            DBUtil.closePreparedStatement(preparedStatement);
            DBUtil.closeConnection(connection);
        }catch (Exception e){
            e.printStackTrace();
            System.out.println("PROFILE插入失败");
        }
    }
    ...
    public void insertSignon(Account account) {
        ...
        try {
            ...
            result= preparedStatement.executeUpdate();
            if (result!=0){
                System.out.println("SIGNON插入成功");
            }
            DBUtil.closePreparedStatement(preparedStatement);
            DBUtil.closeConnection(connection);
        }catch (Exception e){
            e.printStackTrace();
            System.out.println("SIGNON插入失败");
        }
    }
    ...
    public boolean isAccount(String username) {
        ...
        try {
            Connection connection = DBUtil.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(QUERY_ACCOUNT_BY_USERNAME);
    ...
    public void deleteAccount(String username) {
        ...
        try {
            Connection connection = DBUtil.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(DETELE_ACCOUNT);
            preparedStatement.setString(1, username);
            preparedStatement.setString(2, username);
            preparedStatement.setString(3, username);
            result= preparedStatement.executeUpdate();
            if (result!=0){
                System.out.println("Account删除成功");
            }
            DBUtil.closePreparedStatement(preparedStatement);
            DBUtil.closeConnection(connection);
        }catch (Exception e){
            e.printStackTrace();
            System.out.println("Account删除失败");
        }
    ...
    @Override
    public void insertUserBehavior(MyLog myLog) {
        int result ;
        try {
            Connection connection = DBUtil.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(INSERT_MYLOG);
            preparedStatement.setString(1, myLog.getUsername());
            preparedStatement.setString(2, myLog.getBehavior());
            preparedStatement.setString(3, myLog.getTime());
            result= preparedStatement.executeUpdate();
            if (result!=0){
                System.out.println("MYLOG插入成功");
            }
            DBUtil.closePreparedStatement(preparedStatement);
            DBUtil.closeConnection(connection);
        }catch (Exception e){
            e.printStackTrace();
            System.out.println("MYLOG插入失败");
        }
    }

    private static Account resultSetToAccount(ResultSet resultSet) throws Exception{
    ...
    public String getUsernameByEmail(String email) {
        ...
        try {
            Connection connection = DBUtil.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(QUERY_USERNAME_BY_EMAIL);
    ...
    @Override
    public List<MyLog> findAllLog(String username) {
        List<MyLog> myLogList= new ArrayList<>() ;
        try {
            Connection connection = DBUtil.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(QUERY_AllMyLog_BY_USERNAME);
            preparedStatement.setString(1, username);
            ResultSet resultSet = preparedStatement.executeQuery();
            while(resultSet.next()){
                MyLog myLog=new MyLog();
                myLog.setBehavior(resultSet.getString("behavior"));
                myLog.setTime(resultSet.getString("time"));
                myLogList.add(myLog);
            }
            DBUtil.closeResultSet(resultSet);
            DBUtil.closePreparedStatement(preparedStatement);
...
//        }else{
//            System.out.println("faulier");
//        }b
//        boolean is=accountDao.isAccount("s");
//        accountDao.deleteAccount("2");
//        System.out.println(accountDao.getUsernameByEmail("yourname@yourdomain.com"));
        List<MyLog> myLogList= new ArrayList<>() ;
        myLogList=accountDao.findAllLog("songtie");
        for (MyLog myLog:myLogList){
            System.out.print(myLog.getBehavior()+","+myLog.getTime());
            System.out.println();
        }
    }
}